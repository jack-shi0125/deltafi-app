  ...
  return (
    <Wrapper>
      ...
      <PriceImpactWrapper>
        ...
        <PriceList className={isSmall ? "small" : ""}>
          {PRICE_LIST.map(price => (
            <PriceItem key={`item-${price}`} className={`${priceImpact === price ? "active" : ""} ${isSmall ? "small" : ""}`} onClick={() => handleChangeImpact(price)}>
      ...
      <div>
        ...
        <SwitchButton isOn={isIncludeDecimal} handleChange={handleChangeInclude} />
      </div>
    </Wrapper>
  );
};
