...

      {
        ...
        parseFloat(earningAmount) > 0.0 ?
            parseFloat(earningAmount) > maxWithdrawal ?
              <Button name="insufficient" variant='primary' disabled={true} className="connect-wallet" borderRadius="5px">INSUFFICIENT BALANCE</Button>
              :
              <Button name="connect" variant='secondary' className="connect-wallet" borderRadius="5px">APPROVE TRANSACTION</Button>
            :
            <Button name="amount" variant='primary' disabled={true} className="connect-wallet" borderRadius="5px">ENTER AMOUNT</Button>
          :
          <>
            <>
              <Button name="approve" variant='secondary' className="connect-wallet" borderRadius="5px" onClick={() => setHeaderOpen(true)}>
                <ButtonIcon src={'/images/connect-img.png'} alt="Connect" />
                CONNECT WALLET
              </Button>
            </>
            <>
              <Button name="approve" variant='secondary' className="connect-wallet" borderRadius="5px">SUBMIT AGAIN</Button>
              <FlexWrapper className="failed">
                ...
                <Text color={isDark ? "#88809C" : "#6B7280"} className="view" fontFamily="'Inter', sans-serif">
                  <ExternalLink href="/VIEWTRANSACTION" target="_blank" rel="noreferrer noopener">
                    VIEW
